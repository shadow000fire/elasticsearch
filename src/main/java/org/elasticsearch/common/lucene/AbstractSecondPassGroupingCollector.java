 ...
 * 
 * Elasticsearch: The only difference between this and the Lucene version is that the getTopGroups() method can
 * skip empty groups.  In a distributed environment, each shard may not contain elements of the global topN groups.
 *
 * @lucene.experimental
 */
public abstract class AbstractSecondPassGroupingCollector<GROUP_VALUE_TYPE> extends Collector {
  ...
  public TopGroups<GROUP_VALUE_TYPE> getTopGroups(int withinGroupOffset) {
      return getTopGroups(withinGroupOffset,true);
  }
  
  @SuppressWarnings({"unchecked","rawtypes"})
  public TopGroups<GROUP_VALUE_TYPE> getTopGroups(int withinGroupOffset, boolean includeEmpty) {
     
    final GroupDocs<GROUP_VALUE_TYPE>[] groupDocsResult;
    float maxScore = Float.MIN_VALUE;
    if(includeEmpty)
    {
        groupDocsResult = (GroupDocs<GROUP_VALUE_TYPE>[]) new GroupDocs[groups.size()];
    ...
    {
        ArrayList<GroupDocs<GROUP_VALUE_TYPE>> groupList=new ArrayList<GroupDocs<GROUP_VALUE_TYPE>>(groups.size());
        for(SearchGroup<?> group : groups) {
            final SearchGroupDocs<GROUP_VALUE_TYPE> groupDocs = groupMap.get(group.groupValue);
            final TopDocs topDocs = groupDocs.collector.topDocs(withinGroupOffset, maxDocsPerGroup);
            if(topDocs.scoreDocs.length>0)
            {
                groupList.add(new GroupDocs<GROUP_VALUE_TYPE>(Float.NaN,
                                                                          topDocs.getMaxScore(),
                                                                          topDocs.totalHits,
                                                                          topDocs.scoreDocs,
                                                                          groupDocs.groupValue,
        ...
        groupDocsResult=groupList.toArray(new GroupDocs[groupList.size()]);
    }
